	...
	//游戏结束的判断要放在吃食物的判断之前，目的是为了让show发生在push之前(这样可以解决吃掉食物时定时器时间间隔尾巴显示错误)，因为push是发生在吃食物的判断中的
      ...
      if(this.body[0][0]==food.x&&this.body[0][1]==food.y){
		...
		for(var i=0;i<span.length;i++){
			var y=span[i].offsetTop;
			if(span[i].offsetTop==-27){
				span[i].style.transition='0s';
				span[i].style.top='27px';
			}//注意：这里不能写else if，因为要走两个if，如果写else if就只走第一个。
			if(span[i].offsetTop>0){
				span[i].innerHTML=grade;
				span[i].style.transition='1.2s';
				span[i].style.top=0;
				
			}else if(span[i].offsetTop>-27){
				span[i].style.transition='1.2s';
				span[i].style.top='-27px';
			}
		}
